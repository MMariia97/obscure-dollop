...
def annealer(theta, sampler, k, cnf):
   # print("in annealer")
   # if time:
   #     start = time.time()
  #  qubo_sample11 = sampler.sample_qubo(theta, num_reads=k)
   ...
   # print("is soultion valid? ", cnf.is_solution_valid(solution11))
    ...
    while indN<N and not ifsat:
        ...
        print("theta",theta)
        print("BEFORE")
        qubo_sample11 = sampler.sample_qubo(theta, num_reads=k, max_answers = 1)
   ...
   #     print(f"Time: {time.time()-start}")
    ...
    return list(qubo_solution11.values())
...
def gg(Q, A, oldperm, p, n):
  ...
  #          print(len(Q))
        l = inversed[col]
   #         print("l=",l)
        Theta[row, col] = int(0 if Q.get((k,l)) is None else Q.get((k,l)))
    #        print("Q get", int(0 if Q.get((k,l)) is None else Q.get((k,l))))
    

    return Theta, perm


def gg_new(Q, oldperm, p, n):
...
def solve(sampler, d_min, eta, i_max, k, lambda_zero, n, N, N_max, p_delta, q, Q, A, cnf):
 ...
 #   print("sampler prop", sampler.properties)
  #  print("sampler par", sampler.parameters)
    ...
    I = np.identity(n)
    p = 1
    Theta_one, m_one = gg(Q, A, np.arange(n), p, n)
    Theta_two, m_two = gg(Q, A, np.arange(n), p, n)

    start = time.time()
    z_one = map_back(annealer(Theta_one, sampler, k, cnf), m_one)
    convert_1 = datetime.timedelta(seconds=(time.time()-start))
    start = time.time()
    z_two = map_back(annealer(Theta_two, sampler, k, cnf), m_two)
     ...
     #   print(f"---------------------------------------------------------------------------------------------------------------")
       ...
       # print("while true")
        ...
        try:
            ...
            Q_prime_dict = dict(((i,j), Q_prime[i][j]) for i in range(nrow) for j in range(ncol))


            if (i % N == 0):
                p = p - ((p - p_delta)*eta)

            Theta_prime, m = gg(Q_prime_dict, A, m_star, p, n)
            print("theta prime", Theta_prime)
            start = time.time()
            z_prime = map_back(annealer(Theta_prime, sampler, k, cnf), m)
...
#            cnf1.add_constraint_OR(NOT(x[grid[c][0],grid[c][1],s[g][1]]),OR(*[x[c1[0],c1[1],s[g][2]] for c1 in (y for y in grid if manhattan(grid[c],y)==True)]),lam=100)
 ...
 #       qubo_sample11 = sampler.sample_qubo(cnf1.to_qubo().Q, num_reads=2000)
        ...
        A = generate_chimera(max_i)
        qubo_sample11 = solve(sampler, d_min = 7, eta = 0.2, i_max = 200, k = 100, lambda_zero = 3/2, n = max_i, N = 10, N_max = 100, p_delta = 0.1, q = 0.99, Q = Q, A=A, cnf=cnf1)
